...
class UsersModelUser extends JModelAdmin {
    ...
    public function save($data) {
        ...
        $button = JRequest::getVar('button', array(), 'post', 'array');
...
//        $lft = ($obj->lft) + 1;
//        $rgt = ($obj->rgt) + 1;
//        $title = $data['username'] . ' blog';
//        $alias = $data['username'] . '-blog';
//        $path = 'blog/' . $alias;

        $data['button'] = implode(',', $button);
...
//            $user_id = $this->setState('user.id', $user->id);
//            if ($data['id'] == 0 && $button[$key] == 9) {            
//                $catid = $this->insertCategory($lft, $rgt, $path, $title, $alias);
//                $this->insertCatBlog($catid, $user_id);
//                break;
//            }
//        }
        return true;
    ...
    public function getButtons($userId = null) {

        $db = JFactory::getDbo();
        $userId = ($this->getState('user.id')) ? (int) $this->getState('user.id') : "";
        if (!empty($userId)) {
            $query = " SELECT button FROM #__users ";
...
//
//        $db = JFactory::getDbo();
//        $query = "Select * From jos_categories Where parent_id = '87' ORDER BY created_time DESC LIMIT 0,1";
//        $db->setQuery($query);
//        $obj = $db->loadObject();
//        return $obj;
//    }
//
//    public function insertCategory($lft, $rgt, $path, $title, $alias) {
//
//        $db = JFactory::getDbo();
//        $query = "INSERT INTO jos_categories (parent_id, lft, rgt, `level`, path, extension, title, alias, published, access, language) VALUES ('87', $lft, $rgt, 2, '$path', 'com_content', '$title', '$alias', 1, 1, '*') ";
//        $db->setQuery($query);
//        $db->query();
//        $catid = $db->insertid();
//        return $catid;
//    }
//
//    public function insertCatBlog($catid, $user_id) {
//
//        $db = JFactory::getDbo();
//        $query = "INSERT INTO jos_category_blog (catid, user_id) VALUES ($catid, $user_id)";
//        $db->setQuery($query);
//        $db->query();
//        return true;
//    }
