 ...
 */
class PaymentModelPayment4s extends JModelList {
...
//        define("AUTHORIZENET_TRANSACTION_KEY", "552Sr9WK4Mc2mHxq"); // Add your API transaction key9LQ87528hxMdP5jY
        ...
        define("TEST_REQUEST", $config->get('au_test_mode'));           // You may want to set to true if testing against production
        ...
        if ($METHOD_TO_USE == "AIM") {
            ...
            if ($response->approved) {
                ...
                $non_member_price = $config->get('non_member_price');
                $member_price = $config->get('member_price');
                $num_member = $data['options1'];
                $num_non_member = $data['options2'];
                ...
                $data['num_non_mem'] = $num_non_member;

                //$data['price'] = $price;
                $data['total'] = $member_price * $num_member + $non_member_price * $num_non_member;
    ...
    public function storePayment() {
        ...
        $data["category"] = "Reverse Trade";
        $data['num_mem'] = $num_member;
        $data['num_non_member'] = $num_non_member;
        $data['total'] = $member_price * $num_member + $non_member_price * $num_non_member;
        $get_date = JFactory::getDate();
        $date = $get_date->toSql();
        $data['order_date'] = $date;
    ...
    public function insert_user_payment($dt) {
        ...
        $insert->category = $dt['category'];
        $insert->price = $dt['price'];
        //var_dump (JFactory::getDbo()->insertObject('#__payment', $insert));exit();
        try {
