...
class plgSystemOSOLCaptcha extends JPlugin
{
	   ...
	   function onAfterRoute()
        {
				  ...
				  )
				{
					...
					switch($option)
					{
						...
						case 'com_user':
							...
							if(JRequest::getVar('task') == 'registration.register'   && $isEnabledForForm["enableForRegistration"])
							{
			...
			if($isAdmin  )
			{
				...
				if(($paramOsolPassPhrase !='') && ( $sessOsolAdminPassPhrase != $paramOsolPassPhrase)  &&  ($osolPP!= $paramOsolPassPhrase ) )
				{
					...
					JFactory::getApplication()->redirect($liveSiteUserSide);
		...
		function GetCapthcaHTML($vertical = false)
		{
			...
			//echo "HOLA:".JText::_('ENTER CAPTCHA VALUE');
			...
			return ("
					...
					<label for=\"osolCatchaTxt{$GLOBALS['totalCaptchas']}\">".JText::_('ENTER_CAPTCHA_VALUE')."</label> <br />
  ...
  <tr  style=\"border:0px;\">
	...
	".($vertical?"</tr><tr style=\"border:0px;\">":"")."
    <td valign=\"top\"  ".($vertical?"":"width=\"170\"")." style=\"border:0px;\" >
	...
	function reloadCapthcha{$GLOBALS['totalCaptchas']}(instanceNo)
			...
			<label>
       <a href=\"#\" onclick=\"reloadCapthcha{$GLOBALS['totalCaptchas']}(".$GLOBALS['totalCaptchas'].");return false;\" >".JText::_('REFRESH_CAPTCHA')."</a>
    </label></td>
	".($vertical?"</tr><tr style=\"border:0px;\">":"")."
    <td valign=\"top\" style=\"border:0px;\"><input type=\"text\" name=\"osolCatchaTxt\" id=\"osolCatchaTxt{$GLOBALS['totalCaptchas']}\"  class=\"inputbox required validate-captcha\" />&nbsp;
...
");
		...
		function getEnabledForms()
		{
			return array(
									...
									"enableForComLogin"  => '<button type="submit" class="button">'.JText::_('JLOGIN').'</button>', 
									"enableForRegistration" => '<button type="submit" class="validate">'.JText::_('JREGISTER').'</button>',
									//'<button class="button validate" type="submit">'.JTEXT::_('REGISTER').'</button>',
									"enableForReset" => '<button type="submit">'.JText::_('Submit').'</button>',
									"enableForRemind" => '<button type="submit">'.JText::_('Submit').'</button>',
									);
		}
        ...
        function onAfterDispatch()
        {
			...
			if(in_array($option,array('com_contact','com_users')))
			{
				switch($option)
				{
					case 'com_contact':
			...
			if($this->params->get('enableForModules') == 'Yes')// && $option!= 'com_user')
			{
				...
				$templateDir = JPATH_BASE .DS. 'templates'.DS . $app->getTemplate().DS.'index.php';
				//echo $templateDir;
				//<jdoc:include type="modules" name="position-7" style="beezDivision" headerLevel="3" />
				$positions = preg_match_all('#<jdoc:include.+ name="([^"]+)"#iU',
											file_get_contents($templateDir), 
											$matches);
				//echo "<pre>";print_r($matches[1]);echo "</pre>";
				//$type  = $matches[1][$i];
				//echo $type.$name."<br />";
				//foreach(array('left','right','top','user2','user3','loginload','atomic-sidebar','position-7') as $pos)
				foreach(array_unique($matches[1]) as $pos)
				{
					//echo  $pos."<br />";
					...
					$loginModuleSearch ='<input type="submit" name="Submit" class="button" value="'.JText::_('JLOGIN').'" />';
        ...
        {
			global $mainframe;
			//echo( $this->params->get('bgColor')."<hr />");
