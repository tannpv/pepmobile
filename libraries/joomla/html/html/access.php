...
abstract class JHtmlAccess
{
	...
	public static function assetgrouplist($name, $selected, $attribs = null, $config = array())
	{
		...
		return JHtml::_(
           ...
           public static function checkboxbutton($name,$checked) { 
            
//                var_dump($checked[0]);
                $db = JFactory::getDbo();
                
                $button_id_checked = explode(",", $checked[0]->button);           
                $query  =  " SELECT name, id FROM #__usercom  order by name ASC ";
                $db->setQuery($query);
			...
			return null;
                ...
                $html = array();
                $html[] = '<div name =button style = "width:100%">';
                $html[] = " <div class='btn_parent'>" ;
			   $numrow=ceil(count($button)/3);
                for ($i=0, $n=count($button); $i < $n; $i++) { 
                    $checked = (in_array($button[$i]->id, $button_id_checked)) ? "checked" : "";
                    $html[] = " <div class='btn_icon'> 
                        <input type = 'checkbox' $checked value = '".$button[$i]->id."' name ='button[]' id = ".str_replace(" ", "_", $button[$i]->name)." /><label style ='clear: none;' for ='".str_replace(" ", "_", $button[$i]->name)."'>".$button[$i]->name."</label>
                            </div>";
                    if(($i+1) % $numrow == 0 && $i != 0) {
                        $html[] = "</div><div class='btn_parent'>";
                    }
                }
                $html[] = " </div>";
                return implode("\n", $html);
